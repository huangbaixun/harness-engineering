...
> *对于需要数小时乃至数天的任务，需要用 Claude Agent SDK 构建专用 Harness，实现跨上下文窗口的持久工程。*
...
| **发现问题** | Agent 记录在 progress.json 的 notes 里 | Agent 不直接改 features.json，等人决策 |
...
- 发现 features.json 需要修改，在 progress.json 的 notes
...
「在环路外」（Outside the loop）：人类只管理「为什么循环」（我们想要什么），把「如何循环」（怎么构建）完全交给 Agent——即「Vibe Coding」。「在环路中」（In the loop）：人类深入参与最底层的编码循环，手动审查每行 AI 生成的代码——人成为瓶颈。「在环路上」（On the loop）：人类设计和改进控制 Agent 工作的 Harness，而不是亲自执行每一步——这是正确的角色定位。
