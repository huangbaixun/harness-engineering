...
| 机制 | 激活方式 & 最佳适用场景 | 示例 |
|------|---------------------|------|
| **Slash Command** | 手动触发 `/command`；适合有明确起点的工作流 | `/review-pr`, `/deploy-staging` |
...
| **「巨型」单会话** | 一个会话处理 10+ 功能；上下文超 100k 后输出质量下滑 | 每个功能一个会话；用 `/context-dump` 保存进度 |
...
1. 创建 `/review-pr`：PR 提交前的统一审查入口
2. 创建 `/context-dump`：长任务中保存进度到 `claude-progress.json`
...
[PR 阶段]
  → /review-pr Slash Command（触发审查 Sub-agent）
...
```markdown
# .claude/commands/arch-scan.md
---
description: 运行架构健康度扫描并处理发现的问题
---
1. 运行 `bash scripts/arch-scan.sh` 获取架构健康报告
2. 对每个发现的问题评估严重程度：
   - 依赖方向违规：立即修复，提 PR
...
```markdown
# .claude/commands/trim-claudemd.md
---
...
```markdown
# .claude/commands/entropy-scan.md
---
...
|-------|---------|-----------------|
| **writing-plans** | 实现前（>30 分钟 / 3+ 文件） | 规划结果写入 `claude-progress.json.in_progress` |
| **tdd** | 任何代码编写阶段 | 与 Stop Hook（stop-typecheck.sh）形成软硬双层约束 |
| **verification** | 声明完成前 | 语义层补充 Stop Hook 的机械层，对照 `features.json.acceptance_criteria` |
...
```
Superpowers: Brainstorm + writing-plans Skill
        ↓ 人工 Review（对齐意图）
...
writing-plans Skill（整合自 Superpowers）
...
verification Skill（整合自 Superpowers）
...
|---|---|---|
| Using Superpowers（元 Skill） | → `using-harness` 触发规则 Steps 4-6 | 已整合 |
| Writing Plans | → `skills/writing-plans/` | 已整合 |
| Test-Driven Development | → `skills/tdd/` | 已整合 |
| Verification Before Completion | → `skills/verification/` | 已整合 |
...
| Finishing a Development Branch | → `/review-pr` Command 已覆盖部分 | 参考 |
