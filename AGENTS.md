...
> 通用 Agent 记忆文件。支持 Claude Code（读取 CLAUDE.md）、CodeBuddy（读取 CODEBUDDY.md）
...
- Skills/Commands 内容不得硬编码 `.claude/` 或 `.codebuddy/` 路径，统一用 `$TOOL_DIR`
...
- 永远不要在 Skill 内容中硬编码 `.claude/` 或 `.codebuddy/` 路径
