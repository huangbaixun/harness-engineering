...
#### features.json 使用规则（初始化时写入 CLAUDE.md）

**Agent 只读原则**：`features.json` 由人类维护，Agent 绝不直接写入此文件。
- Agent 可以读取 features.json 了解需求和优先级
- 如果 Agent 在工作中发现需求变更或新需求，记录到 `claude-progress.json` 的 `notes` 字段，由人类审核后决定是否更新 features.json
...
`status` 合法值：`planned` → `in_progress` → `done` | `cancelled`（含 `cancelled_reason` 字段）
